...
const ALL = [
  {
    ...
    last: "okay best espresso in town, go",
  },
...
function Matches() {
  ...
  const mutuals = ALL.filter((p) => yes.includes(p.name)).slice(
    0,
    Math.max(1, Math.ceil(yes.length * 0.7)),
  );
        ...
        >
          ...
          <h1 className="mt-4 text-4xl font-semibold sm:text-5xl">
            ...
            <span className="text-gradient">
              {mutuals.length} mutual {mutuals.length === 1 ? "match" : "matches"}
            </span>
            .
          </h1>
        ...
        {mutuals.length === 0 ? (
          <div className="glass-strong mx-auto mt-12 max-w-md rounded-3xl p-10 text-center">
            <p className="text-muted-foreground">
              No mutuals this time. The next round starts in seconds.
            </p>
            <Link
              to="/dashboard"
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-[280px_1fr]">
            <div className="space-y-2">
              {mutuals.map((m, i) => (
                ...
                >
                  <div
                    className={`grid h-12 w-12 place-items-center rounded-full bg-gradient-to-br ${m.color} font-semibold text-white`}
                  >
                    {m.name[0]}
                  </div>
...
function PrivateChat({ name, color }: { name: string; color: string }) {
  ...
  function send() {
    ...
    setDraft("");
    setTimeout(
      () => setMsgs((m) => [...m, { from: "them", text: "haha okay you've got my attention 😄" }]),
      1200,
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 border-b border-white/5 p-4">
        <div
          className={`grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br ${color} font-semibold text-white`}
        >
          {name[0]}
        </div>
        <div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {msgs.map((m, i) => (
          ...
          >
            <div
              className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm ${m.from === "me" ? "gradient-primary text-primary-foreground" : "glass"}`}
            >
              {m.text}
            </div>
          </motion.div>
